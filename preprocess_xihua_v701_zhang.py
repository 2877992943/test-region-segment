...
if __name__=='__main__':
	...
	######
	imarr01_origin=arr01[sample,:,:].copy() 
	pylab.figure();pylab.gray()
	pylab.subplot(1,2,1);pylab.imshow(np.uint8(imarr01_origin))
	######xihua
	imarr01_xihua=try_xihua(arr01[sample,:,:])
	pylab.subplot(1,2,2);pylab.imshow(np.uint8(imarr01_xihua))
	############
	pylab.show()

	'''
	i=0 
	for imarr in arr01_list[:]:
		i+=1
		arr_xh=xihua(imarr) 
		im=Image.fromarray(np.uint8(arr_xh*255.))
		im.save('/home/yr/chinese_char/xihua/xihua_pic_splited_18/'+str(i)+'.jpg') #show effect of 01picture
	'''
